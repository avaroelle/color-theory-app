...
st.set_page_config(page_title="Color Theory Analysis AI", page_icon="🎨", layout="centered")
...
def predict(pil_img, major_model, spring_model, summer_model, autumn_model, winter_model):
...
st.title("🎨 Seasonal Color Analysis AI")
st.write("Upload a photo → Major Season + Subseason + confidence scores.")
...
out = predict(img, major_model, spring_model, summer_model, autumn_model, winter_model)
...
c1, c2 = st.columns(2)
with c1:
    st.markdown("### Major soft probabilities")
    st.dataframe(out["major_soft"], hide_index=True, use_container_width=True)
with c2:
    st.markdown("### Subseason soft probabilities")
